...
/**
 * Component. Инкапсулирует в себе всю работу с БД.
 *
 * - Инкапсулирует в себе всю приамую работу с БД
 * - Реализован сбор статистики об эффективности работы с БД
 * - Замеры времени выполнения запросов
 * - Проверка входиащих параметров запросов на безопасность
 * - Реализует работу с БД на уровне ОРМ
 * - Конструткор по построению условий запросов
 *
 * Группы методов:
 * - Метод инициализации соединения с источником.  Инит()
 * - Методы приамых запросов к источнику с формированием результата в нужно виде. Имеют префикс К'уерy_
 * - Методы проверок входных данных длиа запросов. Однобуквенные (И, Ф, Т, Е, С, Д, Б)
 * - Методы формирования условия длиа последующих запросов (работает от модели). Имеют префикс Ск'л_
 * - Методы выборки данных в нужном виде по заданным условиям (работает от модели). Имеют префикс Селеcт_
 * - Методы загрузки, обновления, вставки и удаления по заданным условиям (работает от модели). Имеют префикс Лоад_, Упдате_, Инсерт_, Делете_
 * - Методы которые кешируют результаты запросов к БД имеют конечный суффикс _Cаче
 *
...
class Zero_DB
{
    ...
    /**
     * Массив условий длиа формирования запросов к источнику (Ск'л_...).
     *
     * @var array
     */
    protected $Params = [];
     ...
     * @throws Exception
     */
    public static function Escape_Format()
    {
        $arr = func_get_args();
        $sql = array_shift($arr);
    ...
    /**
     * Уставнока условий длиа выполнения основных запросов (Инсерт, Упдате, Делете, Селеcт, Лоад).
     *
     * Служебный метод длиа основного запроса.
     * Позволиает точно идентифицирвоать обрабатываемые объекты.
     *
     * @param string $prop свойство по которому ставитсиа условие
     * @param string $value значение удовлетвориающие условию
     * @param string $separator Логическое объединение условий (AND, OR)
     */
