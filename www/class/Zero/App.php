 ...
 */
define('ZERO_PATH_LOG', ZERO_PATH_SITE . '/logs');
/**
 * Location cache
 */
 ...
 */
define('ZERO_PATH_CLASS', ZERO_PATH_SITE . '/class');
/**
...
class Zero_App
{
    ...
    public static function Autoload($class_name)
    {
        if ( class_exists($class_name) )
            return true;
        if ( file_exists($path = ZERO_PATH_SITE . '/class/' . str_replace('_', '/', $class_name) . '.php') )
        {
            require_once $path;
            return true;
        }
    ...
    {
        //  Include Components
        require_once ZERO_PATH_CLASS . '/Zero/Config.php';
        require_once ZERO_PATH_CLASS . '/Zero/Session.php';
        require_once ZERO_PATH_CLASS . '/Zero/Cache.php';
        require_once ZERO_PATH_CLASS . '/Zero/Logs.php';
        require_once ZERO_PATH_CLASS . '/Zero/Route.php';
        ...
        require_once ZERO_PATH_CLASS . '/Zero/View.php';
    ...
    public static function Execute()
    {
        ...
        if ( '' != self::$Route->Route[self::$Route->Url] )
        {
            $_REQUEST['act'] = 'Action_' . $_REQUEST['act'];
            //
            $Controller = Zero_Controller::Factory(self::$Route->Route[self::$Route->Url]);
            Zero_Logs::Start('#{CONTROLLER.Action} ' . self::$Route->Route[self::$Route->Url] . ' -> ' . $_REQUEST['act']);
            $view = $Controller->$_REQUEST['act']();
            if ( $_REQUEST['act'] != 'Action_Default' )
                Zero_Logs::Set_Message_Action($_REQUEST['act']);
            Zero_Logs::Stop('#{CONTROLLER.Action} ' . self::$Route->Route[self::$Route->Url] . ' -> ' . $_REQUEST['act']);
        }

        // Основные данные
        $viewLayout = new Zero_View('Index');
    ...
    public static function Execute_Bak()
    {
    ...
    /**
     * Обработчик исключений длиа функции set_exception_handler()
     *
     * - '403' стандартный ответ на закрытый раздел (страницу сайта)
     * - '404' стандартный ответ не найденного документа
     * - '500' все остальные критические ошибки приложения либо сервера
     *
    ...
    /**
     * Профилирование работы приложения при его завершении
     *
     * - Сбор всех таймеров и затраченной памиати
     * - Замер полного времени выполнения приложения
     * - Вывод всей профилированной информации в указанные источники
     */
